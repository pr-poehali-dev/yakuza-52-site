...
import { playersApi, tournamentsApi, Player, Tournament } from '@/lib/api';
...
export default function HomePage() {
  ...
  const clanStats = {
    ...
    tournaments: tournaments.filter(t => t.status === 'completed').length,
  };
    ...
    <Layout>
      ...
      <section className="relative min-h-[80vh] flex items-center justify-center hero-grid overflow-hidden">
        ...
        <div className="text-center px-4 animate-fade-in">
          ...
          <h1 className="font-oswald text-6xl md:text-8xl font-bold text-white mb-3 glow-purple-text">YAKUDZA</h1>
          <h2 className="font-oswald text-3xl md:text-5xl font-bold text-purple-400 mb-6 tracking-widest">52</h2>
      ...
      <section className="max-w-5xl mx-auto px-4 -mt-10 mb-16">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { label: 'Бойцов', value: clanStats.members, icon: 'Users', color: 'text-purple-400' },
            { label: 'Побед', value: clanStats.wins.toLocaleString(), icon: 'Trophy', color: 'text-yellow-400' },
            { label: 'Убийств', value: clanStats.kills.toLocaleString(), icon: 'Target', color: 'text-red-400' },
            { label: 'Турниров', value: clanStats.tournaments, icon: 'Award', color: 'text-green-400' },
          ].map((stat) => (
            <div key={stat.label} className="card-glass rounded-xl p-5 text-center animate-scale-in">
              <Icon name={stat.icon} size={28} className={`${stat.color} mx-auto mb-2`} />
        ...
        <section className="max-w-5xl mx-auto px-4 mb-16">
          ...
          <div className="grid md:grid-cols-3 gap-4">
            {top3.map((player, i) => {
              ...
              const podiumColors = [
                'from-yellow-600/30 to-yellow-900/10 border-yellow-500/30',
                'from-gray-500/20 to-gray-800/10 border-gray-500/30',
                'from-amber-700/20 to-amber-900/10 border-amber-600/30',
              ];
              const medals = ['👑', '🥈', '🥉'];
              return (
            ...
            {upcoming.map((t) => (
              <div key={t.id} className="card-glass rounded-xl p-5 border-purple-glow">
                <div className="flex items-start justify-between mb-3">
                  <span className="text-xs bg-green-500/10 text-green-400 border border-green-500/20 rounded-full px-2 py-0.5">Скоро</span>
                ...
                {t.description && <p className="text-muted-foreground text-sm mb-3 line-clamp-2">{t.description}</p>}
                <div className="flex items-center justify-between text-sm">
                  <span className="text-muted-foreground">
                    <Icon name="Users" size={13} className="inline mr-1" />
                    {t.participants}/{t.maxParticipants}
                  </span>
                  {t.prize && <span className="text-yellow-400 text-xs"><Icon name="Gift" size={13} className="inline mr-1" />{t.prize}</span>}
                </div>
              </div>
            ))}
          </div>
