...
export default function RatingPage() {
  ...
  useEffect(() => {
    playersApi.list().then(({ players: p }) => { setPlayers(p); setLoading(false); }).catch(() => setLoading(false));
  }, []);
  ...
  const avgKD = players.length
    ? (players.reduce((s, p) => s + p.kills / Math.max(p.deaths, 1), 0) / players.length).toFixed(2)
    : '0.00';
