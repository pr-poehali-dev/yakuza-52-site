...
export default function ProfilePage() {
  ...
  const [form, setForm] = useState({ standoffId: '', bio: '', region: '' });
  ...
  const startEdit = () => {
    setForm({ standoffId: currentUser.standoffId || '', bio: currentUser.bio || '', region: currentUser.region || '' });
    setEditMode(true);
  };
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      await playersApi.update(currentUser.id, { standoffId: form.standoffId, bio: form.bio, region: form.region });
      await refreshUser();
      setEditMode(false);
    } catch (e: unknown) {
      ...
      <div className="max-w-4xl mx-auto px-4 py-12">
        <div className="card-glass rounded-2xl p-8 mb-6">
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6">
            ...
            <div className="flex-1 min-w-0">
              ...
              {currentUser.bio && !editMode && <p className="text-muted-foreground text-sm mb-3">{currentUser.bio}</p>}
              <div className="flex flex-wrap gap-4 text-sm text-muted-foreground">
                {currentUser.region && (
                  <span className="flex items-center gap-1.5"><Icon name="MapPin" size={14} />{currentUser.region}</span>
                )}
                {currentUser.joinedAt && (
                  <span className="flex items-center gap-1.5"><Icon name="Calendar" size={14} />В клане с {currentUser.joinedAt}</span>
                )}
                {currentUser.standoffId && (
                  <span className="flex items-center gap-1.5"><Icon name="Gamepad2" size={14} />{currentUser.standoffId}</span>
                )}
              </div>
            </div>
          ...
          {editMode && (
            <div className="mt-6 pt-6 border-t border-border space-y-4">
              <div>
                ...
                <input
                  type="text"
                  value={form.standoffId}
                  onChange={e => setForm({ ...form, standoffId: e.target.value })}
        ...
        <div className="card-glass rounded-xl p-6">
          ...
          <div className="space-y-4">
            {[
              { label: 'Процент побед', value: wr, color: 'bg-green-400' },
              { label: 'K/D (макс 5.0)', value: Math.min(parseFloat(kd) * 20, 100), color: 'bg-purple-400' },
              { label: 'Рейтинговые очки', value: Math.min(Math.round((currentUser.points / 20000) * 100), 100), color: 'bg-yellow-400' },
            ].map((bar) => (
              <div key={bar.label}>
